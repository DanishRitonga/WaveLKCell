...
Following WaveConvX~\cite{waveconvx}, we integrate a multi-level wavelet enhancement module at the bottleneck of our architecture, applied to the deepest encoder features $\mathbf{F}_3 \in \mathbb{R}^{C \times H' \times W'}$ where $C=384$. This module decomposes features into frequency sub-bands, applies the frequency-specific processors from WaveConvX~\cite{waveconvx}, and reconstructs enhanced features.

\subsubsection{Two-Level Haar DWT}

Following WaveConvX~\cite{waveconvx}, we apply two successive levels of the 2D Haar Discrete Wavelet Transform (DWT). The first level decomposes the input into four sub-bands:
\begin{equation}
...
We implement the Haar DWT efficiently using strided convolutions with fixed wavelet kernels, grouped by channel to ensure independent transformation per channel with minimal computational overhead, as described in WaveConvX~\cite{waveconvx}.

\subsubsection{Frequency-Specific Processing}

Following the design of~\cite{waveconvx}, each frequency sub-band is processed by a specialized module (\cref{fig:wavelet}):

\textbf{High Frequency Processor (HH$_1$) --- Adaptive Power Gabor Convolution (APGConv)~\cite{waveconvx}:}
...
\textbf{Mid-High Frequency Processor (LH$_1$, HL$_1$) --- Self-Attention~\cite{waveconvx}:}
The LH$_1$ and HL$_1$ sub-bands contain transitional edge structures. Following WaveConvX~\cite{waveconvx}, we apply multi-head self-attention ($4$ heads) to capture long-range spatial dependencies in these frequency components, followed by batch normalization and GELU activation. Unlike WaveConvX~\cite{waveconvx}, which averages the LH$_1$ and HL$_1$ outputs, we process them independently with shared weights to preserve directional information required by the subsequent IDWT reconstruction.

\textbf{Second Wavelet Decomposition:}
The LL$_1$ approximation coefficients from the first-level DWT are further decomposed by a second DWT into $\{\text{LL}_2, \text{LH}_2, \text{HL}_2, \text{HH}_2\}$ at resolution $H'/4 \times W'/4$.

\textbf{Mid Frequency Processor (HH$_2$) --- Depthwise Pointwise Convolution:}
...
\textbf{Mid-Low Frequency Processor (LH$_2$, HL$_2$) --- Channel Attention~\cite{waveconvx}:}
The second-level horizontal and vertical detail bands are processed by squeeze-and-excitation style channel attention~\cite{waveconvx}, which recalibrates feature responses across channels to emphasize informative frequency components, followed by a $3{\times}3$ convolution.

\textbf{Low Frequency Processor (LL$_2$) --- Convolution:}
