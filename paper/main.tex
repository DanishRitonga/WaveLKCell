...
% ====== BOTTLENECK (bottom, same column as encoder) ======
\node[wav, minimum width=2.4cm] (we) at (0, -5.8) {Wavelet\\[-2pt]Enhance\\[-2pt]{\tiny(\cref{fig:wavelet})}};

\node[wav, minimum width=2.0cm] (ds) at (0, -7.0) {Downsample\\[-2pt]{\tiny $384{\to}768$ch}};

% ====== DECODER (right column, going UP from bottom) ======
\node[dec] (d3) at (4.2, -5.8) {Dec Block 3 {\tiny $K{=}13$}\\[-2pt]{\scriptsize $\mathbf{F}_4 \to 1024$ch}};

\node[dec] (d2) at (4.2, -4.4) {Dec Block 2 {\tiny $K{=}27$}\\[-2pt]{\scriptsize $512$ch}};

\node[dec] (d1) at (4.2, -3.0) {Dec Block 1 {\tiny $K{=}29$}\\[-2pt]{\scriptsize $256$ch}};

\node[dec] (up2) at (4.2, -1.6) {Up $+$ Skip\\[-2pt]{\scriptsize $128$ch, $H/2$}};

\node[dec] (up1) at (4.2, -0.2) {Up $+$ Skip\\[-2pt]{\scriptsize $64$ch, $1{\times}$}};

% ====== HEADS (top right) ======
\node[head] (np) at (7.2, -0.2) {NP Head\\[-2pt]{\tiny 2ch}};
\node[head] (hv) at (7.2, -1.1) {HV Head\\[-2pt]{\tiny 2ch}};
\node[head] (nt) at (7.2, -2.0) {NT Head\\[-2pt]{\tiny $N_c$ ch}};
...
% Encoder -> Wavelet -> Downsample (continue down same column)
\draw[arr] (s3) -- (we);
\draw[arr] (we) -- (ds);

% Bottom U-turn: downsample across to decoder
\draw[arr] (ds.east) -| (d3.south);
...
\draw[skip, draw=gray!40] (stem.east) -- ++(1.0,0) |- node[near end, above, font=\tiny, text=gray!60] {input skip} (up2.west);
\draw[skip, draw=gray!40] (input.east) -- ++(2.0,0) |- node[near end, above, font=\tiny, text=gray!60] {input skip} (up1.west);

% ====== ANNOTATIONS ======
\node[font=\scriptsize\bfseries, text=blue!60!black, rotate=90, anchor=south] at (-1.5, -3.0) {Encoder};
\node[font=\scriptsize\bfseries, text=orange!60!black, rotate=90, anchor=south] at (-1.5, -6.4) {Bottleneck};
\node[font=\scriptsize\bfseries, text=green!50!black, rotate=-90, anchor=south] at (5.6, -3.0) {Decoder};
