...
def main() -> None:
    ...
    trainer = WaveLKCellTrainer(
        ...
        lr=overrides.get("lr", opt_cfg.get("lr", 8e-4)),
        backbone_lr_ratio=opt_cfg.get("backbone_lr_ratio", 0.1),
        betas=tuple(opt_cfg.get("betas", [0.85, 0.95])),
        weight_decay=opt_cfg.get("weight_decay", 0.05),
        epochs=overrides.get("epochs", trainer_cfg.get("epochs", 130)),
        accumulate_grad_batches=trainer_cfg.get("accumulate_grad_batches", 1),
        gradient_clip_val=trainer_cfg.get("gradient_clip_val", 0.1),
        warmup_epochs=trainer_cfg.get("warmup_epochs", 0),
        eta_min=sched_cfg.get("eta_min", 1e-5),
        unfreeze_backbone_at_epoch=trainer_cfg.get("unfreeze_backbone_at_epoch", 25),
