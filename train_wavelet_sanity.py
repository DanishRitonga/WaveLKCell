"""Wavelet sanity check: LKCell baseline with Stage 3 replaced by WaveletEnhance.

Takes the proven LKCell baseline (train_lkcell_baseline.py) and surgically
replaces Stage 3 (3x UniRepLKNetBlock at 768-dim) with MultiWaveletEnhance(384)
+ Conv3s2(384→768) downsample. Everything else is identical.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Wavelet Sanity Check")
...
def create_wavelet_cellvit(device="cuda"):
    """Create CellViT with Stage 3 replaced by WaveletEnhance."""
    model = CellViT(
        model256_path=None,
        num_nuclei_classes=6,
    ...
    wavelet_enhance = MultiWaveletEnhance(384).to(device)
    wavelet_downsample = nn.Sequential(
...
def main():
    ...
    save_dir = Path(f"runs/wavelet_sanity/{run_name}")
